...
const Profile = ({ navigation }: { navigation: ProfileNavigation }) => {
  ...
  return (
    <SafeAreaView style={tw("p-6 flex-1 bg-[#FCFCFC]")}>
      <Text style={{ fontFamily: "Bold", ...tw("mb-4 text-2xl") }}>
        Witaj <Text style={tw("text-primary")}>{username}!</Text>
      </Text>
      <Pressable
        onPress={() => navigation.navigate("OwnFlashCards")}
        style={tw("bg-blue-400 py-3 px-6 my-2")}
      >
        <Text style={tw("text-white font-medium")}>Dodane fiszki</Text>
      </Pressable>
      <Pressable
        onPress={() => navigation.navigate("FlashLists")}
        style={tw("bg-blue-400 py-3 px-6 my-2")}
      >
        <Text style={tw("text-white font-medium")}>FiszkoListy</Text>
      </Pressable>
      <Pressable onPress={handleLogout}>
        <Text style={tw("text-red-400 mt-4 font-medium")}>Wyloguj</Text>
      </Pressable>
    </SafeAreaView>
  );
};
