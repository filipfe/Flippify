...
const CardForm = ({ navigation }: { navigation: CardFormNavigationProp }) => {
  ...
  if (status)
    return (
      <View style={tw("p-6")}>
        <Text style={{ fontFamily: "SemiBold", ...tw("text-xl") }}>
          {status}
        </Text>
      </View>
    );
  ...
  return (
    <View style={tw("p-6 flex-1 bg-[#FCFCFC] items-center")}>
      <SelectDropdown
        ...
        renderCustomizedButtonChild={(sel) => (
          <View style={tw("items-center")}>
            ...
            {sel && (
              <Text style={{ fontFamily: "Bold", ...tw("text-primary") }}>
                {sel}
              </Text>
            )}
          </View>
        ...
        buttonStyle={tw(
          `w-full px-6 ${
            newCard.category ? "h-20" : "h-16"
          } items-center mb-6 border-stroke border-[2px] rounded-2xl bg-white`
        )}
          ...
          <View style={tw("items-center")}>
            <Text
              style={{
                ...
                ...tw(
                  `text-lg ${
                    !newCard.category ? "text-[#B6C3B9]" : "text-black"
                  }`
                ),
              }}
            >
      ...
      <SelectDropdown
        ...
        renderCustomizedButtonChild={(sel) => (
          <View style={tw(`items-center`)}>
            ...
            {sel && (
              <Text style={{ fontFamily: "Bold", ...tw("text-primary mt-1") }}>
                {sel}
              </Text>
            )}
          </View>
        ...
        buttonStyle={tw(
          `w-full ${
            newCard.type ? "h-20" : "h-16"
          } px-6 items-center mb-6 border-stroke border-[2px] rounded-2xl bg-white`
        )}
        dropdownStyle={tw("rounded-2xl bg-white")}
        onSelect={(item) =>
          setNewCard((prev: AddedFlashCardProps) => ({
      ...
      <Pressable
        ...
        style={tw(
          `items-center w-full justify-center px-6 ${
            newCard.question ? "h-20" : "h-16"
          } mb-6 border-stroke border-[2px] rounded-2xl bg-white`
        )}
      >
        <Text
          style={{
            fontFamily: "Bold",
            ...tw(`text-lg ${!newCard.type ? "text-[#B6C3B9]" : "text-black"}`),
          }}
        >
          Konstruuj pytanie
        </Text>
...
type QuestionsFormNavigationProp = NavigationProp<
  AddCardStackParams,
  "QuestionsForm"
>;
...
const QuestionsForm = ({
  ...
  return (
    <View style={tw("p-6 flex-1")}>
      <View
        style={tw(
          "w-full px-10 items-center mb-6 py-4 border-stroke border-[2px] items-center rounded-2xl bg-white"
        )}
      >
        <Text style={{ fontFamily: "Bold", ...tw("text-lg mb-2") }}>
          Wprowadź treść pytania
        </Text>
        <View style={tw("border-b-[#E3E8E4] border-b-[3px] py-1")}>
          <TextInput
            style={{ fontFamily: "Bold", ...tw("text-lg text-primary") }}
      ...
      <View style={tw("relative mb-4")}>
        <TextInput
          ...
          style={{
            fontFamily: "SemiBold",
            ...tw(
              "py-3 px-6 border-[3px] text-lg mt-3 rounded-2xl relative z-10 w-full bg-white border-[#E3E8E4]"
            ),
          }}
          onChangeText={(text) =>
            setCorrectAnswer({
        ...
        <View
          style={tw(
            "absolute left-0 right-0 h-[2rem] bg-[#E3E8E4] -bottom-[0.4rem] rounded-b-2xl"
          )}
        />
      </View>
      {newCard.type === "radio" &&
      ...
      {newCard.type === "radio" && wrongAnswers.length < 3 && (
        <View style={tw("relative mb-4")}>
          <Pressable
            ...
            style={tw(
              "bg-white relative border-[3px] border-[#E3E8E4] z-10 rounded-2xl w-full mx-auto py-4 px-12"
            )}
          >
            <Text
              style={{ fontFamily: "Bold", ...tw("text-[1.1rem] mx-auto") }}
            >
              Dodaj złą odpowiedź
            </Text>
          </Pressable>
